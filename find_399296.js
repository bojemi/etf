...
const run = async () => {
  ...
  const target = 3.47;
  const datesToCheck = data.slice(-2).map(d => d.date); // Today and yesterday
  
  const xs = Array.from({length: 10}, (_, i) => i + 1); // 1 to 10
  const ys = Array.from({length: 51}, (_, i) => i + 10); // 10 to 60
  ...
  for (const date of datesToCheck) {
    ...
    console.log(`\nDate: ${date} - Top 5 matches for target ${target}%:`);
    for (let i = 0; i < 5; i++) {
