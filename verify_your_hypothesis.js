...
const run = async () => {
  ...
  console.log('--- 验证：现价 / 近3日最高收盘价(不含当日) - 1 ---');
  for (const etf of etfs) {
    ...
    // 近3日最高收盘价 (不含当日，即 T-1, T-2, T-3)
    // 也就是从 index-1 开始往前数 3 天
    const max3_excluding_today = calculateMax(data, index - 1, 3);
    
    const calc = (currentPrice / max3_excluding_today - 1) * 100;

    console.log(`${etf.name}:`);
    console.log(`  目标值: ${etf.targetStop}%`);
    console.log(`  计算值: ${calc.toFixed(2)}%`);
    console.log(`  (现价: ${currentPrice}, 前3日最高: ${max3_excluding_today})`);
    console.log('------------------------');
  }
};
